...
<section class="hero_single version_2" style="background: #222 url(<?php echo asset('uploads/system/home_banner.jpg'); ?>) center center no-repeat; background-size: cover;">
    <div class="wrapper">
        <div class="container">
            <h3><?php echo App\FrontendSetting::all()->keyBy('type')['banner_title']->description; ?>!</h3>
            <p><?php echo App\FrontendSetting::all()->keyBy('type')['slogan']->description; ?></p>
...
//            $categories = $this->db->get_where('category', array('parent' => 0))->result_array();
            foreach ($categories as $key => $category):?>
            <div class="col-lg-4 col-md-6">
                <a href="<?php echo url('home/filter_listings?category='.Illuminate\Support\Str::slug($category['name'],'-').'&&amenity=&&video=0&&status=all'); ?>" class="grid_item">
                    <figure>
                        <img src="<?php echo asset('uploads/category_thumbnails/').$category['thumbnail'];?>" alt="">
                        <div class="info">
                            <small><?php echo count($category->listings).' '.'Listings'; ?></small>
                            <h3><?php echo $category['name']; ?></h3>
                        </div>
                    </figure>
                </a>
        ...
        <div class="item">
            <div class="strip grid">
                ...
                <div class="wrapper">
                    <h3>
                        <a href="<?php echo route('listings.show',$listing['id']); ?>" class="float-left"><?php echo $listing['name']; ?></a>
                        <?php $claiming_status = (App\ClaimedListing::where('listing_id',$listing['id'])->first()->status); ?>
                        <?php if($claiming_status == 1): ?>
                        <img class="float-left ml-1" data-toggle="tooltip" title="This listing is verified'); ?>" src="<?php echo asset('frontend/images/verified.png'); ?>" style="width: 25px;">
                        <?php endif; ?>
                    </h3>
                ...
                <!-- <li><span class="loc_open"><?php echo ($listing->time); ?></span></li> -->
                    ...
                    <li><div class="score"><span>
						<?php
                                if ($listing->reviews->first()->review_rating > 0) {
                                    $quality = $listing->reviews;
                                    echo $quality;
                                }else {
                                    echo 'Unreviewed';
                                ...
                                ?>
						<em><?php echo count($listing->reviews).' '.'Reviews'; ?></em></span><strong><?php echo $listing->reviews; ?></strong></div></li>
                </ul>
            </div>
        </div>
        <?php endforeach; ?>
